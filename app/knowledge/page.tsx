    ...
    <div className="space-y-6 max-w-4xl mx-auto font-sans p-4">
      <div className="flex justify-between items-center bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Daftar seluruh instruksi dan aturan yang dipelajari AI.</p>
        </div>
        <button onClick={fetchKnowledge} className="px-3 py-1.5 bg-blue-50 text-blue-600 rounded-xl text-xs font-semibold hover:bg-blue-100">
          🔄 Refresh
        </button>
      </div>
        ...
        <div className="grid grid-cols-1 gap-3">
          {knowledgeList.map((item) => (
            <div key={item.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-start gap-4">
              ...
              <button onClick={() => handleDelete(item.id)} className="text-red-500 hover:text-red-700 text-xs font-bold">
                🗑️ Hapus
              </button>
            </div>
          ))}
